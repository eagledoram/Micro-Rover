...
void commandDecoder()
{
  ...
  #if bluetooth
  while(Serialx.available()){  // note: Leonardo does not support serialEvent() handler
     data[i] = Serialx.read();
  #else
     ...
     uint8_t   eeprom_data[16];        // 16 bytes maximum
      ...
      if (cmd == MOVE) {
        ...
        ***********************************/
        ...
        /*
        WARNING
        WARNING ONLY THE TB6612FNG and L298N HAS BEEN UPDATED WITH NEW INDEX ADDRESSES
